    ...
    if elem goal ctx then  -- Use-Assumption rule
      ...
      case goal of
        ...
        And phi1 phi2     -> -- And-Right rule
                             prove' ctx phi1 (\()-> prove' ctx phi2 (kSucceed, kFail), kFail)
                                              
        Or phi1 phi2      -> -- Or-Right rules (try Or-Right-1, if it fails, try Or-Right-2)
                             prove' ctx phi1 (kSucceed, \()-> prove' ctx phi2 (kSucceed, kFail))
    ...
    in
      case focus of
        ...
        Or phi1 phi2 ->        -- Or-Left rule
            prove_left original (done, [phi1] ++ rest) goal (leave_alone, \()-> prove_left original (done, [phi2] ++ rest) goal (kSucceed, kFail))
